...
void loop() {
 ...
 if (abs(angleX) >= 85 && abs(angleX) <= 95) {
    Serial.println("Device is VERTICALLY STANDING on X-axis (90 degrees)");
} 
if (abs(angleX) >= 175 && abs(angleX) <= 185) {
    Serial.println("Device is UPSIDE-DOWN (180 degrees on X-axis)");
} 
if (abs(angleY) >= 85 && abs(angleY) <= 95) {
    Serial.println("Device is VERTICALLY STANDING on Y-axis (90 degrees)");
} 
if (abs(angleY) >= 175 && abs(angleY) <= 185) {
    Serial.println("Device is FLIPPED 180 degrees on Y-axis");
}
